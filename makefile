...
$(TARGET)\Cadencii.exe: resources .\Cadencii\*.cs $(TARGET)\org.kbinani.dll $(TARGET)\org.kbinani.apputil.dll $(TARGET)\org.kbinani.media.dll $(TARGET)\org.kbinani.vsq.dll $(TARGET)\PlaySound.dll
	gmcs -recurse:.\Cadencii\*.cs -define:MONO,USE_DOBJ,ENABLE_VOCALOID;ENABLE_AQUESTONE;ENABLE_MIDI -out:$(TARGET)\Cadencii.exe -r:System.Windows.Forms,System.Drawing,$(TARGET)\org.kbinani.dll,$(TARGET)\org.kbinani.apputil.dll,$(TARGET)\org.kbinani.media.dll,$(TARGET)\org.kbinani.vsq.dll -unsafe+ -codepage:utf8 $(MCS_OPT)
