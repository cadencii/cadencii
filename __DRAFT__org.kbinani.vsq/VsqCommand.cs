...
namespace org.kbinani.vsq {
    ...
    public class VsqCommand {
        ...
        /*public static VsqCommand generateCommandTrackRemovePoints( int track, String target, List<Long> ids ) {
            VsqCommand command = new VsqCommand();
            command.Type = VsqCommandType.TRACK_CURVE_REMOVE_POINTS;
            command.Args = new Object[3];
            command.Args[0] = track;
            command.Args[1] = target;
            List<Long> cpy = new List<Long>();
            if ( ids != null ){
                int count = ids.size();
        ...
        /// <returns></returns>
        public static VsqCommand generateCommandTrackCurveEdit2( int track, String target, List<Long> delete, Dictionary<Integer, VsqBPPair> add ) {
            VsqCommand command = new VsqCommand();
            ...
            Dictionary<Integer, VsqBPPair> cp_add = new Dictionary<Integer, VsqBPPair>();
        ...
        /// <param name="add_or_move">追加または移動を行うデータ点のリスト</param>
        /// <returns></returns>
        public static VsqCommand generateCommandTrackCurveEdit2All( int track, List<String> target, List<List<Long>> delete, List<Dictionary<Integer, VsqBPPair>> add ) {
            VsqCommand command = new VsqCommand();
            ...
            List<Dictionary<Integer, VsqBPPair>> cp_vec_add = new List<Dictionary<Integer, VsqBPPair>>();
            c = add.size();
            for ( int i = 0; i < c; i++ ) {
                Dictionary<Integer, VsqBPPair> cp_add = new Dictionary<Integer, VsqBPPair>();
                Dictionary<Integer, VsqBPPair> tmp = add.get( i );
