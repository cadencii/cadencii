...
if( org.kbinani.Cp932 == undefined ){
    ...
    org.kbinani.Cp932._get_char_from_table = function(ch, cl) {
        ...
        if (n == -1 || n >= org.kbinani.Cp932.jis_table.length){
            return "?";
        }
        return org.kbinani.Cp932.jis_table.charAt(n);
    };

    org.kbinani.Cp932._get_code_from_table = function(c) {
        var p, ch, cl;
        if ((p = org.kbinani.Cp932.jis_table.indexOf(c)) == -1){
            return -1;
        }
        ch = parseInt(p / 0x5e);
        cl = (p % 0x5e) + 0x21;
    ...
    org.kbinani.Cp932._sjis_to_jis = function(k1, k2) {
        k1 = (k1 << 1) & 0xff;
        if (k2 < 0x9f) {
     ...
     */
    org.kbinani.Cp932.convertFromUTF = function( s ){
        var r = new Array();
        var c, p, n;
        for (var i = 0; i < s.length; i++) {
            ...
            if (c <= 127){
                ...
                p = org.kbinani.Cp932._get_code_from_table(s.charAt(i));
                if (p == -1) {
                    r.push( 0 );
                } else {
                    n = org.kbinani.Cp932._jis_to_sjis(p >> 8, p & 0xff);
                    r.push( n >> 8 ); 
                    r.push( n & 0xff );
                }
            }
     ...
     */
    org.kbinani.Cp932.convertToUTF8 = function( byte_array ) {
        var c, ch = 0, r = "", p;
        for (var i = 0; i < byte_array.length; i++) {
            ...
            if (ch == 0) {
                if ((c >= 0x81 && c <= 0x9f) || (c >= 0xe0 && c <= 0xfc)){
                    ...
                    r += String.fromCharCode( byte_array[i] );
                }
            } else {
                if ((c >= 0x40 && c <= 0x7e) || (c >= 0x80 && c <= 0xfc)) {
                    p = org.kbinani.Cp932._sjis_to_jis(ch, c);
                    r += org.kbinani.Cp932._get_char_from_table(p >> 8, p & 0xff);
                } else {
                    r += "?"; 
                }
                ch = 0;
