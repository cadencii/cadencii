...
public class Hamori : System.Windows.Forms.Form {
    ...
    private static void hamori( com.github.cadencii.vsq.VsqFile vsq, int basecode, int opt ) {
        ...
        for ( com.github.cadencii.java.util.Iterator<SelectedEventEntry> itr = AppManager.itemSelection.getEventIterator(); itr.hasNext(); ){
        ...
        if ( opt == 0 ) {
            for ( int j = 0; j < vsq.Track.get( track ).getEventCount(); j++ ) {
                com.github.cadencii.vsq.VsqEvent item = vsq.Track.get( track ).getEvent( j );
                if ( item.ID.type == com.github.cadencii.vsq.VsqIDType.Anote && target_ids.ContainsKey( item.InternalID ) ) {
